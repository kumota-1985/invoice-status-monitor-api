...
> ⚠️ **データは組み込みのモック（3社）であり、実際の国税庁「適格請求書発行事業者公表システム」のデータではありません。** デモ・連携テスト用です。本番化には実データソース連携が必要です。なお Webhook URL は SSRF 対策として登録時・送信時に検証され、内部/プライベートアドレスは拒否されます。
